...
const bygningSchema = z.object({
  bygningsnummer: z.number().nonnegative().or(z.string()).meta({
  ...
  lopenummer: z.number().int().nonnegative().nullable().optional().meta({
    title: "Lopenr",
    example: 1,
  }),
  ...
  bygningsstatuser: z
    .record(z.string(), z.iso.datetime({ offset: true }).nullable().optional())
    .optional(),
