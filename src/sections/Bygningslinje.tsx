      ...
      <Table zebra border>
        ...
        <Table.Body>
          {bygning.endringer.map((endring) => {
            const erFerdigstilt =
              endring.datoer.ferdigattest !== null ||
              endring.datoer.tattIBruk !== null
            const utgattKode = endring.bygningsstatus.bestaaende
              ? "-"
              : endring.bygningsstatus.kortkode
            const harSefrakminner = endring.kulturminner.some(
              (k) => k.kategori === SEFRAK_KATEGORI,
            )
            const harEnkeltminner = endring.kulturminner.some(
              (k) => k.kategori !== SEFRAK_KATEGORI,
            )
            ...
            return (
              <Table.Row key={endring.id}>
                ...
                <Table.Cell>{utgattKode}</Table.Cell>
                ...
                <Table.Cell>{erFerdigstilt ? "Ja" : "Nei"}</Table.Cell>
                <Table.Cell>
                  {`${harSefrakminner ? "Ja" : "Nei"} / ${
                    harEnkeltminner ? "Ja" : "Nei"
                  }`}
                </Table.Cell>
              </Table.Row>
            )
